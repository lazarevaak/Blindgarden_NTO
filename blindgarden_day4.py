...
class Task:
	...
	def line_detect(self, image):
		...
		if compute_rect[0] != float('inf'):
			...
			if y_min > (height - y_min) and self.first_reverse:
				print('reverse line')
...
TAKEOFF = True
QR_CODE = True
#QR_CODE_FAKE = [2.5, 2.0]
LINE_DETECT = True
LAND = True

# сегодня не будем записывать видео, так что указываем 
# record = False
task = Task(record = False, output_file = 'newday4.mp4')
...
if QR_CODE:
	...
	if len(task.nav_area) == 0:
		...
		for i in range(5):
			for x, y in square:
				task.flight.navigate_block(x = x, y= y, z = height_landing, frame_id = 'aruco_map', speed = 0.3, tolerance = 0.2)
				
				ts = rospy.Time.now()
				while len(task.nav_area) == 0:
					if (rospy.Time.now() - ts) > rospy.Duration(secs = 3.0):
						break
					rospy.sleep(0.2)
...
if LAND:
	...
	time.sleep(1.0)
	print('Start landing with corection')
